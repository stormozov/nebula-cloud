...
describe("Form components", () => {
  ...
  /**
   * Snapshot tests for visual regression across configurations.
   */
  describe("snapshot tests", () => {
    it("Form default matches snapshot", () => {
      /**
       * @description Snapshot for default Form
       * @scenario Render basic Form
    ...
    it("FormRow default matches snapshot", () => {
      /**
       * @description Snapshot for FormRow
       * @scenario Render FormRow with children
       * @expected Matches snapshot
       */
      const { container } = render(
        <Form.Row>
          <label htmlFor="snap-input">Label</label>
          <input id="snap-input" />
        </Form.Row>,
      );
      expect(container.firstChild).toMatchSnapshot();
    ...
    it("FormSubmitErrorBlock with errors matches snapshot", () => {
      /**
       * @description Snapshot for error block shown
       * @scenario Render with errors
       * @expected Matches snapshot
       */
      const { container } = render(<Form.SubmitErrorBlock errors="Error" />);
      expect(container.firstChild).toMatchSnapshot();
    });

    it("Full compound Form usage matches snapshot", () => {
      /**
       * @description Snapshot for compound usage
       * @scenario Form with Row and ErrorBlock
       * @expected Complete form structure snapshot
       */
      const { container } = render(
        <Form className="test-form" onSubmit={vi.fn()}>
          <Form.Row>
            <input />
          </Form.Row>
          <Form.SubmitErrorBlock errors="Submit error" />
        </Form>,
      );
      expect(container.firstChild).toMatchSnapshot();
    });
